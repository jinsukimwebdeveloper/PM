    ...
    public class TicketDataAccess : ITicketDataAccess
    {
        ...
        public int CreateIssue(CreateIssueResult createIssueResult)
        {
            ...
            using (DbCommand cmd = db.GetStoredProcCommand("[dbo].[CreateIssue]"))
            {
                ...
                db.AddInParameter(cmd, "@Project", DbType.Int32, createIssueResult.Project);
        ...
        public IEnumerable<ProjectResult> GetProject()
        {
            Database db = new DatabaseProviderFactory().Create("JIRA");
            ...
            using (DbCommand cmd = db.GetStoredProcCommand("[dbo].[GetProjectSelectList]"))
            {
                ...
                for (int i = 0; i < ds.Tables[0].Rows.Count; i++)
                {
                    ...
                    project.Id = (int)ds.Tables[0].Rows[i]["Id"];
                    project.Subject = (string)ds.Tables[0].Rows[i]["Subject"];
                    result.Add(project);
                }
            }
            return result;
