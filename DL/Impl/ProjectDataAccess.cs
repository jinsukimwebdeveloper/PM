...
namespace Jira.DL.Impl
{
    public class ProjectDataAccess : IProjectDataAccess
    {
        ...
        public ProjectResult FindProject(int id)
        {
            Database db = new DatabaseProviderFactory().Create("JIRA");
            using (DbCommand cmd = db.GetStoredProcCommand("[dbo].[FindProject]"))
            {
                cmd.CommandTimeout = dbTimeout;
                ...
                result.Subject = DBNull.Value.Equals(ds.Tables[0].Rows[0]["Subject"]) ? string.Empty : (string)ds.Tables[0].Rows[0]["Subject"];
                result.Status = DBNull.Value.Equals(ds.Tables[0].Rows[0]["Status"]) ? string.Empty : (string)ds.Tables[0].Rows[0]["Status"];
                result.Description = DBNull.Value.Equals(ds.Tables[0].Rows[0]["Description"]) ? string.Empty : (string)ds.Tables[0].Rows[0]["Description"];
                result.StartTime = DBNull.Value.Equals(ds.Tables[0].Rows[0]["StartTime"]) ? DateTime.MinValue : (DateTime)ds.Tables[0].Rows[0]["StartTime"];
                result.EndTime = DBNull.Value.Equals(ds.Tables[0].Rows[0]["EndTime"]) ? DateTime.MinValue : (DateTime)ds.Tables[0].Rows[0]["EndTime"];
                result.RecentSprint = DBNull.Value.Equals(ds.Tables[0].Rows[0]["RecentSprint"]) ? string.Empty : (string)ds.Tables[0].Rows[0]["RecentSprint"];
                result.ReleasedVersion = DBNull.Value.Equals(ds.Tables[0].Rows[0]["ReleasedVersion"]) ? string.Empty : (string)ds.Tables[0].Rows[0]["ReleasedVersion"];
                result.SourceRespository = DBNull.Value.Equals(ds.Tables[0].Rows[0]["SourceRespository"]) ? string.Empty : (string)ds.Tables[0].Rows[0]["SourceRespository"];
